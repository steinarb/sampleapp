...
class LocalizationResourceTest {
    ...
    void testAvailableLocales() {
        var sampleapp = mock(SampleappService.class);
        when(sampleapp.defaultLocale()).thenReturn(NB_NO);
        when(sampleapp.availableLocales()).thenReturn(Arrays.asList(Locale.forLanguageTag("nb-NO"), Locale.UK).stream().map(l -> LocaleBean.with().locale(l).build()).toList());
        var resource = new LocalizationResource();
        resource.sampleapp = sampleapp;
    ...
    void testDisplayTextsForDefaultLocale() {
        ...
        var displayTexts = (Map<String, String>)resource.displayTexts(sampleapp.defaultLocale().toString()).getEntity();
